export type BackdropMode = "aurora-glass" | "midnight-ocean" | "liquid-prism" | "solar-frost" | "sci-fi-grid" | "lunar-waves" | "cartoon-beach" | "nebula-rain";
export type ThemeMode = "dark" | "light" | "system" | undefined;
export type BackdropPalette = "codex" | "graphite" | "midnight" | "ember" | "absolute" | "paper" | "dawn" | "contrast";
export type CursorLight = "off" | "subtle" | "strong";

export interface AnimatedBackdropOptions {
  mode?: BackdropMode;
  theme?: ThemeMode;
...
const paletteAccents: Record<BackdropPalette, [string, string, string]> = {
...
function drawPointerLight(ctx: CanvasRenderingContext2D, pointer: PointerState, width: number, height: number, time: number, colors: string[], cursorLight: CursorLight) {
...
export function startAnimatedBackdrop(canvas: HTMLCanvasElement, options: AnimatedBackdropOptions = {}) {
  const ctx = canvas.getContext("2d", { alpha: true });
  if (!ctx) return () => {};

  const mode = options.mode ?? "aurora-glass";
